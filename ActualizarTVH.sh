...
do
	if [ "$#" -eq 0 ]; then
		clear
		echo -e "\n\t-Para \033[0;31mreiniciar\033[0m toda la configuración de TVHeadend, selecciona \033[0;31mR\033[0m (Borra canales, redes y tags)\n\
\t-Para realizar una \033[0;31mactualización\033[0m, sin borrar configuraciones que hayas hecho, selecciona \033[0;31mA\033[0m (Permite conservar configuraciones como la TDT)\n\
\t-Para \033[0;31msalir\033[0m, selecciona \033[0;31mS\033[0m\n"
		read -p $'Reiniciar, Actualizar o Salir \033[0;31m(R|A|S)\033[0m?: ' opcion
	else
		...
		opcion="A" #Actualización silenciosa. No borra nada
	...
	case $opcion in
		...
		cd "$CARPETA_DESCARGA"
		rm -f $ACTUALIZACION_ZIP
		
		# Descargamos el fichero de actualización
		MostrarMensaje "Descargamos el fichero de actualización: $ACTUALIZACION_ZIP" "WHITE"
		wget ${ncert} -q "$ACTUALIZACION_URL$ACTUALIZACION_ZIP" 2>&1 | tee $LOG_FILE
		if [ ! -f $ACTUALIZACION_ZIP ]; then
			MostrarError "Error en la descarga del fichero $ACTUALIZACION_URL$ACTUALIZACION_ZIP"	
		else
			...
			if [ ! -f  untvh.pyo ]; then
				...
				MostrarMensaje "Descomprimimos el fichero $CARPETA_DESCARGA/$ACTUALIZACION_ZIP"
				#tar -xf "$CARPETA_DESCARGA/$ACTUALIZACION_ZIP" -C $CARPETA_TEMPORAL
				cd "$CARPETA_DESCARGA"
				python untvh.pyo "$CARPETA_DESCARGA/$ACTUALIZACION_ZIP" $CARPETA_TEMPORAL
				if [ ! -d "$CARPETA_TEMPORAL/storage" ] ; then
					MostrarError "Error al descomprimir $ACTUALIZACION_ZIP"
				else
					...
					rm -f "$CARPETA_DESCARGA/$ACTUALIZACION_ZIP"
...
function ChequearInstalar() {
	...
	CuentaAtras "Comprobación de versión y posible instalación" 4
	...
	if [ "$#" -eq 1 ]; then pause;fi # Si nos pasan un parámetro, hacemos una pausa (no hacemos pausa cuando nos pasan argumento al script)
}
...
function AddNormandy(){

	dir1="/storage/.kodi/userdata/addon_data/service.tvheadend42/channel/config/*"
	dir2="/storage/.kodi/userdata/addon_data/service.tvheadend42/channel/tag/*"
	cadena="NormandyEPG"

	for fichero in $dir1 $dir2
	do 

	   if [ -f "$fichero" ]; then  
		 ultima=$(tail -n 1 $fichero)

		 if [[ "$ultima" != $cadena ]]; then
		   echo $cadena | tee -a $fichero >/dev/null
		   echo $fichero
		 fi
	   fi
	done
...
if [[ $# -eq 1 ]]; then
	...
	case $1 in
		...
		-DEBUG|-debug) # No documentado. Para utilizar el servidor de pruebas
			...
			MenuPrincipal
			;;
		-ADDNORMANDY|-addnormandy) # añadimos la marca Normandy a los ficheros de la guía
